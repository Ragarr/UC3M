...
class SList:
    ...
    def remove_pattern(self):
        ...
        while current and current.next:
            if current.elem*2==current.next.elem:
                current.next=self._remove_pattern(current)
            current=current.next
        return leng-self.size


    def _remove_pattern(self,node:SNode):
        if node: # para evitar que se pase un none 
            if node.next and node.elem*2!=node.next.elem:
                return node.next
            self.size-=1
            return self._remove_pattern(node.next)

            
l=SList()
data=[4, 1, 2, 4, 8, 9, 3, 2, 5, 10, 20, 2, 1, 7]
for i in range(10):
    l.addFirst(i)
l.addFirst(8)
l.addFirst(4)
...
l.addLast(8)
print(l)
print(l.remove_pattern())
print(l)
l2=SList()
for i in data:
    l2.addLast(i)
print(l2)
print(l2.remove_pattern())
print(l2)
l3=SList()
print(l3.remove_pattern())