    ...
    */
    ...
    while (1){
        ...
        if (procesar_operacion(elem) < 0){   // si la operacion es erronea, terminar ejecucion del thread
            ...
            printf("Error procesando la operacion %d, proceso finalizado\n", my_bank_numop+1);
            exit(-1);
        }
...
int comprobar_n_operaciones(char *vec) {
    ...
    */
    ...
    char *cabecera;
    if (vec == NULL) {
        printf("Error: no se ha podido proyectar el fichero");
        return -1;
    }; // si no se ha podido proyectar el fichero, salir del programa
    if (vec[0] == '\0') {
        printf("Error: fichero vacio");
        return -1;
    } // si el fichero esta vacio, salir del programa
    // contar cuanto ocupa la cabecera (donde esta el numero de operaciones)
    i = 0;
    while (vec[i] != '\n') {
        i++;
    }
    cabecera = malloc((i+1)*sizeof(char));    // reservar memoria para la cabecera
    strncpy(cabecera, vec, i);                // copiar la cabecera en el vector cabecera
    cabecera[i] = '\0';                       // añadir el caracter de fin de string
    n_op = atoi(cabecera);                    // convertir la cabecera a entero
...
int crear_cuenta(int cuenta_origen){
    ...
    */
    ...
    if (cuenta_origen < 0 || cuenta_origen > MAX_CUENTAS){
        printf("Error: el numero de cuenta no es valido\n");
        res = -1;
    }
    else{
