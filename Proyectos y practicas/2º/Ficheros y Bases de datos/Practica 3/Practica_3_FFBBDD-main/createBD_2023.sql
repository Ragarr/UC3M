    ...
    DROP INDEX searchk_indx;
    --DROP CLUSTER albums_tracks_clus;

    -- -----------------------------------------
    -- CUSTER (BAJA EL RENDIMIENTO)
    -- -----------------------------------------
    --CREATE CLUSTER albums_tracks_clus(PAIR CHAR(15))
    --   TABLESPACE TAB_16K;
    ...
    --CLUSTER albums_tracks_clus(PAIR);
    ...
    -- ÍNDICE DEL CLUSTER QUE BAJABA EL RENDIMIENTO:
    -- Índice del cluster de albums y tracks
    --CREATE INDEX albums_tracks_indx ON CLUSTER albums_tracks_clus;

    -- Creación de dos columnas en ‘tracks’ y valuación de una de ellas
    ALTER TABLE tracks add (searchk varchar2(20), lyrics VARCHAR2(4000)); 
    UPDATE tracks set searchk=pair||'//'||sequ; 
    COMMIT;

    -- ÍNDICE DE SEARCHK
    CREATE INDEX searchk_indx ON TRACKS(searchk);

    -- ALTERACIÓN DEL TAMAÑO DEL BLOQUE
    ALTER TABLE tracks MOVE TABLESPACE TAB_16K;
    ALTER TABLE albums MOVE TABLESPACE TAB_16K;
    ALTER TABLE performers MOVE TABLESPACE TAB_16K;
    ALTER TABLE songs MOVE TABLESPACE TAB_16K;
    ALTER TABLE PERFORMANCES MOVE TABLESPACE TAB_16K;
    COMMIT;
